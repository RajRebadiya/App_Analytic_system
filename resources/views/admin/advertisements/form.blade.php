...
<form class="cardx p-4" method="POST" enctype="multipart/form-data" action="{{ $advertisement->exists ? route('admin.advertisements.update', $advertisement) : route('admin.advertisements.store') }}">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-2"><label class="form-label">Start</label><input class="form-control" type="text" data-flatpickr-datetime name="start_date" value="{{ old('start_date', optional($advertisement->start_date)->format('Y-m-d H:i')) }}" placeholder="Start Date" autocomplete="off"></div>
        <div class="col-md-2"><label class="form-label">End</label><input class="form-control" type="text" data-flatpickr-datetime name="end_date" value="{{ old('end_date', optional($advertisement->end_date)->format('Y-m-d H:i')) }}" placeholder="End Date" autocomplete="off"></div>
